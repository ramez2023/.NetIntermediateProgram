...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            var matrixA = new Matrix(100, 100);
            var matrixB = new Matrix(100, 100);

            // Measure regular multiplication time
            Stopwatch regularTimer = Stopwatch.StartNew();
            var resultRegular = regularMultiplier.Multiply(matrixA, matrixB);
            regularTimer.Stop();
            long regularTime = regularTimer.ElapsedMilliseconds;

            // Measure parallel multiplication time
            Stopwatch parallelTimer = Stopwatch.StartNew();
            var resultParallel = parallelMultiplier.Multiply(matrixA, matrixB);
            parallelTimer.Stop();
            long parallelTime = parallelTimer.ElapsedMilliseconds;
